...
A user sent a bad.news URL that displayed a link preview thumbnail in Telegram. The bot tried to download the HLS video, found every available variant too large for Telegram upload, and then reported the HLS failure instead of saving the visible preview image. During follow-up review, we also found that successful URL video downloads were sent back to Telegram without storing a searchable thumbnail image in Typesense. A later check showed that Telegram-generated thumbnails for URL videos can be poor local preview evidence, so URL videos should prefer the source webpage preview image instead.

## Root cause

The existing thumbnail fallback only handled media that Telegram includes in Bot API updates with a downloadable `file_id`, such as message photos or media thumbnails. For plain text URL messages, Telegram can show a client-side link preview while the Bot API update only includes `link_preview_options.url`; it does not include a thumbnail `file_id`. The fallback therefore returned `:no_thumbnail` even though the source webpage exposed a preview image through Open Graph metadata and the video poster. Separately, the `.mp4` send path returned after `sendVideo` and never indexed a searchable preview image. When that was first fixed by preferring the Telegram `sendVideo` thumbnail, URL videos could still end up with thumbnails that did not display reliably. HLS success had one more gap: it sent the downloaded video without passing the original source URL into the shared video-send path, so preview indexing had no webpage URL and fell through to `:no_thumbnail`.
...
Seeing a preview in Telegram does not guarantee that Bot API delivered a downloadable thumbnail. Link fallback should treat Telegram message media and webpage preview metadata as separate recovery sources. Video success and video failure paths both need explicit searchable-image handling; sending media to Telegram is not the same as indexing it. For URL videos, the source page preview is better search evidence than Telegram's generated upload thumbnail; Telegram thumbnails should remain the primary source only for direct video uploads.
