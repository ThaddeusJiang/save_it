defmodule SaveIt.DownloadedFileNameTest do
  use ExUnit.Case, async: true

  alias SaveIt.DownloadedFileName

  test "generates uuidv7 filename with the original URL extension" do
    file_name = DownloadedFileName.random("https://example.com/downloads/video.mp4?token=abc")

    assert_uuidv7_filename(file_name, ".mp4")
  end

  test "generates a fresh uuidv7 filename for each call" do
    first = DownloadedFileName.random("https://example.com/photo.jpg")
    second = DownloadedFileName.random("https://example.com/photo.jpg")
  ...
  test "uses the fallback extension when the original has no extension" do
    file_name =
      DownloadedFileName.random("https://example.com/download", fallback_extension: ".jpeg")
